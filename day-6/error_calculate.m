...
for i = 2 : length(rho)-1
  ...
  %Fill First order forward difference approximation here%
  rho_prime_central_difference(i, 1) = (rho(i+1, 1) - rho(i-1, 1)) / (2 * (x(i+1, 1) - x(i, 1)) ) ;
  %Fill Second order central difference approximation here%
end
